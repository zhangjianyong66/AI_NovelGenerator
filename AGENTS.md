...
- 生成任务接口为 `POST /api/generation-jobs`、`GET /api/projects/{project_id}/jobs`、`GET /api/generation-jobs/{job_id}`；当前支持 `architecture`、`directory`、`draft`、`finalization`、`consistency` 和 `batch`，任务状态、日志、错误和创建请求参数保存到本地 SQLite 状态库 `.local/state.sqlite3`。`.local/` 已被 `.gitignore` 忽略；删除该库只会清空任务历史，不会删除小说输出文件。`batch` 请求支持 `startChapter`、`endChapter`、`targetWords`、`minimumWords`、`autoEnrich`。
