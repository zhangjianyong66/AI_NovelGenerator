...
- 新前端要从“可编辑工作台”变成“可真实创作工具”时，优先级记录在 `docs/feature-map-and-acceptance.md` 的“新前端真实可用开发顺序”：真实生成执行器最小闭环、章节草稿/定稿闭环、任务持久化与可恢复工作流、知识库真实向量化、项目管理和章节生命周期最小闭环已完成；后续不要优先扩大纯视觉 UI 重构，应继续补齐审校、批量、项目配置隔离等真实能力。
