...
- 定稿阶段从 `Novel_directory.txt` 读取当前/下一章目录时必须使用 `chapter_directory_parser.get_chapter_outline_context(...)`，该 helper 兼容 `# 第N章 - 标题` 等 Markdown 章节标题；不要再用只识别行首 `第N章` 的正则。`finalize_chapter(...)` 写回前会检查模型返回正文第一条非空行的显式章节号，若定稿第 N 章却返回“第 M 章”会拒绝覆盖原章节文件，避免批量定稿把章节整体写偏。
