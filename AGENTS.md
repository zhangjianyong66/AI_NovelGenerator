...
- 新前端要从“可编辑工作台”变成“可真实创作工具”时，优先级记录在 `docs/feature-map-and-acceptance.md` 的“新前端真实可用开发顺序”：真实生成执行器最小闭环、章节草稿/定稿闭环、任务持久化与可恢复工作流、知识库真实向量化、项目管理、章节生命周期最小闭环和一致性审校真实执行已完成；后续不要优先扩大纯视觉 UI 重构，应继续补齐批量、项目配置隔离等真实能力。
...
- 本地 SQLite 状态库 `.local/state.sqlite3` 保存生成任务历史和最近项目索引，`.local/` 已被 `.gitignore` 忽略；删除该库只会清空任务历史和最近项目列表，不会删除 `config.json` 或小说输出文件。
...
- 生成任务的 `architecture`、`directory`、`draft`、`finalization` 和 `consistency` 阶段已通过 `app/services/generation_executor.py` 接入同步真实执行器：`architecture` 调用旧 `Novel_architecture_generate(...)` 后同步 `Novel_architecture.txt` 到前端读取的 `Novel_setting.txt`，`directory` 调用旧 `Chapter_blueprint_generate(...)` 并写入 `Novel_directory.txt`，`draft` 调用旧 `generate_chapter_draft(...)` 生成章节草稿，`finalization` 调用旧 `finalize_chapter(...)` 更新 `global_summary.txt`、`character_state.txt` 并沿用旧逻辑尝试更新向量库，`consistency` 调用旧 `check_consistency(...)` 读取小说设定、角色状态、全局摘要、剧情要点和章节正文并把审校结果写入任务日志且不修改项目文件；批量仍是待后续接入的任务记录边界。
