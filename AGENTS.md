...
- 生成任务接口为 `POST /api/generation-jobs`、`GET /api/projects/{project_id}/jobs`、`GET /api/generation-jobs/{job_id}`；当前支持 `architecture`、`directory`、`draft`、`finalization`、`consistency` 和 `batch`，任务状态、日志、错误和创建请求参数保存到本地 SQLite 状态库。`batch` 请求支持 `startChapter`、`endChapter`、`targetWords`、`minimumWords`、`autoEnrich`。
- 生成任务的 `architecture`、`directory`、`draft`、`finalization`、`batch` 和 `consistency` 阶段已通过 `app/services/generation_executor.py` 接入同步真实执行器：`architecture` 调用旧 `Novel_architecture_generate(...)` 后同步 `Novel_architecture.txt` 到前端读取的 `Novel_setting.txt`，`directory` 调用旧 `Chapter_blueprint_generate(...)` 并写入 `Novel_directory.txt`，`draft` 调用旧 `generate_chapter_draft(...)` 生成章节草稿，`finalization` 调用旧 `finalize_chapter(...)` 更新 `global_summary.txt`、`character_state.txt` 并沿用旧逻辑尝试更新向量库，`batch` 第一版只对范围内已有 `chapter_X.txt` 执行批量定稿，单章失败后继续后续章节并在任务日志汇总成功/失败章节，`consistency` 调用旧 `check_consistency(...)` 读取小说设定、角色状态、全局摘要、剧情要点和章节正文并把审校结果写入任务日志且不修改项目文件。
