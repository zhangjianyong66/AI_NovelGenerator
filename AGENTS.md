...
- 新前端要从“可编辑工作台”变成“可真实创作工具”时，优先级记录在 `docs/feature-map-and-acceptance.md` 的“新前端真实可用开发顺序”和“后续任务路线”：真实生成执行器最小闭环、章节草稿/定稿闭环、任务持久化与可恢复工作流、知识库真实向量化、项目管理、章节生命周期最小闭环、一致性审校真实执行、批量草稿/定稿/审校真实执行和项目参数隔离第一版已完成；后续不要优先扩大纯视觉 UI 重构，应按顺序补齐生成任务后台执行、失败任务重试与批量失败重跑、任务中断恢复边界、每项目模型配置覆盖、章节生命周期状态补强。
...
- 章节编辑页和工作台切换当前章节时，会把选中章节号同步写入后端项目配置 `novelParams.chapterNum` / `config.json.other_params.chapter_num`；生成任务页的草稿、定稿和审校目标章节仍以该后端项目配置为准，不直接读取浏览器工作上下文作为权威章节号。
...
- 本地 API 已提供 `GET /health`、`GET /api/projects`、`POST /api/projects`、`POST /api/projects/switch`、`GET/PUT /api/project-config`、`GET/PUT /api/model-settings` 和 `POST /api/model-settings/test-llm`；项目创建会创建指定输出目录并写入当前 `config.json.other_params.filepath/topic/genre/num_chapters/word_number`，项目切换会把当前输出路径切到最近项目或已有目录；项目配置接口继续读写根目录 `config.json` 的 `other_params.filepath`、`topic`、`genre`、`num_chapters`、`word_number`、`chapter_num`、`user_guidance`、`characters_involved`、`key_items`、`scene_location`、`time_constraint`，响应给前端时使用 camelCase 字段。
...
- 当 `config.json.proxy_setting.enabled` 为 `false` 时，项目语义是未启用代理；`llm_adapters.py` 构造 OpenAI 兼容 LLM 客户端时会显式使用 `httpx` 的 `trust_env=False`，避免进程继承的系统 `HTTP_PROXY`、`HTTPS_PROXY`、`ALL_PROXY` 或 `OPENAI_PROXY` 污染生成执行。后续如需支持项目级显式代理，应把 `proxy_setting` 明确传入 adapter，不要依赖系统环境变量。
- `llm_adapters.py` 构造 OpenAI 兼容 LLM 客户端时统一设置 `User-Agent: AI-NovelGenerator/1.0 (local desktop app; OpenAI-compatible)`；部分中转接口会拦截 OpenAI Python SDK 默认 `User-Agent`，不要把该头改回 SDK 默认值。
...
- 章节接口为 `GET /api/projects/{project_id}/chapters`、`POST /api/chapters/{chapter_number}` 和 `PUT /api/chapters/{chapter_number}`；列表会合并输出目录已有 `chapter_<数字>.txt` 与从 `Novel_directory.txt` / `num_chapters` 推导的计划章节，缺失文件的章节返回 `planned` 状态和空正文；`POST` 只创建缺失的根部章节文件且不会覆盖已有正文，`PUT` 只保存已存在章节，并尝试从 `Novel_directory.txt` 补章节标题和简述。
- 本地 SQLite 状态库 `.local/state.sqlite3` 保存生成任务历史、最近项目索引和最近项目参数快照，`.local/` 已被 `.gitignore` 忽略；删除该库只会清空任务历史、最近项目列表和项目参数快照，不会删除 `config.json`、模型密钥或小说输出文件。
- 生成任务接口为 `POST /api/generation-jobs`、`GET /api/projects/{project_id}/jobs`、`GET /api/generation-jobs/{job_id}` 和 WebSocket `WS /api/projects/{project_id}/generation-jobs/ws`；当前支持 `architecture`、`directory`、`draft`、`finalization`、`consistency`、兼容旧批量定稿的 `batch`，以及明确批量阶段 `batchDraft`、`batchFinalization`、`batchConsistency`，任务状态、日志、错误和创建请求参数保存到本地 SQLite 状态库。批量请求支持 `startChapter`、`endChapter`、`targetWords`、`minimumWords`、`autoEnrich`。
- 生成任务的 `architecture`、`directory`、`draft`、`finalization`、`batch`、`batchDraft`、`batchFinalization`、`batchConsistency` 和 `consistency` 阶段已通过 `app/services/generation_executor.py` 接入后台真实执行器；`POST /api/generation-jobs` 创建任务后会尽快返回 `queued` 或 `running` 初始快照，后端后台线程更新 SQLite 状态库，并通过 WebSocket 推送 `generationJobUpdated` 完整任务快照，前端不应假设创建响应已经是最终 `done`/`failed`。`architecture` 调用旧 `Novel_architecture_generate(...)` 后同步 `Novel_architecture.txt` 到前端读取的 `Novel_setting.txt`，`directory` 调用旧 `Chapter_blueprint_generate(...)` 并写入 `Novel_directory.txt`，`draft` 调用旧 `generate_chapter_draft(...)` 生成章节草稿，`finalization` 调用旧 `finalize_chapter(...)` 先基于前章结尾、当前/下一章目录信息润色章节正文并写回 `chapter_X.txt`，其中前章结尾默认取前一章最后 3 个自然段且最多 1500 字，再更新 `global_summary.txt`、`character_state.txt` 并沿用旧逻辑尝试更新向量库，`batch`/`batchFinalization` 对范围内已有 `chapter_X.txt` 执行批量定稿，`batchDraft` 对范围内缺失章节生成草稿且遇到已有 `chapter_X.txt` 会跳过不覆盖，`batchConsistency` 对范围内已有章节逐章审校，批量任务单章失败后继续后续章节并在任务日志汇总成功/失败章节，`consistency` 调用旧 `check_consistency(...)` 读取小说设定、角色状态、全局摘要、剧情要点和章节正文并把审校结果写入任务日志且不修改项目文件。
- 本地 API 启动时会把 SQLite 中遗留的 `queued` / `running` 生成任务标记为 `failed`，`progress=100`，错误和日志写入“后端服务重启，任务已中断”；后台线程不会跨服务重启恢复，前端刷新后不应继续展示这些旧任务为运行中。
- 定稿阶段从 `Novel_directory.txt` 读取当前/下一章目录时必须使用 `chapter_directory_parser.get_chapter_outline_context(...)`，该 helper 兼容 `# 第N章 - 标题` 等 Markdown 章节标题；不要再用只识别行首 `第N章` 的正则。`finalize_chapter(...)` 写回前会检查模型返回正文第一条非空行的显式章节号，若定稿第 N 章却返回“第 M 章”会拒绝覆盖原章节文件，避免批量定稿把章节整体写偏；若返回的是当前章号标题行（如 `# 第N章 标题`），写回前会剥离该标题行，保持章节正文文件不混入额外标题。
- 项目参数隔离第一版通过 `.local/state.sqlite3` 的最近项目参数快照实现；`GET/PUT /api/project-config` 和 `POST /api/projects/switch` 会同步/恢复 `other_params` 中的输出目录、题材、类型、章节数、每章字数、当前章节、用户指导、涉及角色、关键物品、场景和时间约束。打开尚无参数快照的已有目录时，只切换输出路径并清空项目参数，避免继承上一个项目的题材和章节设置。根目录 `config.json` 仍是旧 GUI 使用的当前活动配置；LLM、Embedding、阶段模型、代理、WebDAV、API Key 和密码仍全局共享，不做项目级隔离。
- 章节草稿/定稿执行器为兼容旧 GUI 与新前端，会在输出目录根部 `chapter_<数字>.txt` 和旧函数使用的 `chapters/chapter_<数字>.txt` 之间同步正文；前端章节接口仍以根部 `chapter_<数字>.txt` 为读取和保存入口。当前新前端定稿不自动更新 `plot_arcs.txt`。
- 知识工具接口为 `GET /api/knowledge`、`POST /api/knowledge/import`、`POST /api/knowledge/clear-vectorstore`、`GET /api/knowledge/plot-arcs`；导入会读取当前 `config.json` 选中的 Embedding 配置，把指定本地文件分块写入当前输出目录的 Chroma `vectorstore/`，成功后复制源文件到 `vectorstore/imported/` 作为导入记录；缺少 Embedding 配置、模型名或远程接口 API Key 时返回中文错误。清理会删除整个 `vectorstore/`，剧情要点读取 `plot_arcs.txt`；知识列表会汇总 `vectorstore/imported/` 下的导入文件和 `角色库/<分类>/<角色名>.txt` 角色文件，导入文件在检测到向量库数据时显示“已向量化”标签。
