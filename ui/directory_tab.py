...
def build_directory_tab(self):
    ...
    load_btn = ctk.CTkButton(self.directory_tab, text="加载 Novel_directory.txt", command=self.load_chapter_blueprint, font=UI_FONT)
    load_btn.grid(row=0, column=0, padx=5, pady=5, sticky="w")

    self.directory_word_count_label = ctk.CTkLabel(self.directory_tab, text="字数：0", font=UI_FONT)
    self.directory_word_count_label.grid(row=0, column=1, padx=5, pady=5, sticky="w")

    save_btn = ctk.CTkButton(self.directory_tab, text="保存修改", command=self.save_chapter_blueprint, font=UI_FONT)
    save_btn.grid(row=0, column=2, padx=5, pady=5, sticky="e")

    self.directory_text = ctk.CTkTextbox(self.directory_tab, wrap="word", font=EDITOR_FONT)
