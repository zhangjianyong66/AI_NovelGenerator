...
| 未接入阶段生成任务创建成功 | `batch` 返回 `queued`，日志说明等待执行器接入，不提示真实生成完成 |
...
- 前端源码/合约测试应覆盖生成任务页的真实后端边界：目标章节提示、草稿不要求预先存在章节文件、审校要求已有章节正文、批量缺章提示、后端错误 `detail` 透传、`queued` 详情解释。
- 真实后端冒烟：
  - 保存项目输出目录。
  - 工作台保存核心项目文件并检查真实文件落盘。
  - 章节页保存已存在 `chapter_1.txt` 并检查真实文件落盘。
  - 创建已接入阶段生成任务并确认 `done` / `failed`、错误和日志展示。
  - 创建未接入批量任务并确认 `queued` 与等待执行器日志。
- 离线预览冒烟：
...
- 生成任务页：通过后端创建任务。已接入阶段包括设定、目录、草稿、定稿和审校，会同步真实执行并返回 `done` / `failed`；审校结果写入任务日志且不自动修改小说文件。未接入阶段包括批量，只创建 queued 任务，不承诺生成或修改小说文件。
