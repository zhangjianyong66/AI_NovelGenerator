...
当前项目只有一个最小 SQLite 状态库用于保存前端生成任务历史；核心业务数据仍由本地 `config.json`、输出目录文本文件、角色库目录、WebDAV 配置备份和 Chroma `vectorstore/` 承载。项目没有 ORM 或迁移系统。
...
如果后续 milestone 要把 SQLite 扩大到项目、章节、知识库或其他业务数据，必须先更新设计文档和本规范，并明确：
