...
| 项目管理 | 新建、打开、切换本地小说项目 | 通过配置输出路径间接支持 | 真实支持最近项目和当前项目切换 | `GET /api/projects`、`POST /api/projects`、`POST /api/projects/switch`，当前项目仍写入 `config.json.other_params.filepath` | 在项目页创建临时项目、打开已有目录，检查工作台读取对应文件 |
...
重要边界：新前端生成任务当前“小说设定”“章节目录”“章节草稿”和“章节定稿”已接入同步真实执行器；一致性审校和批量生成仍只创建任务记录，不会调用 LLM 或修改小说文件。生成任务历史、日志、错误、创建请求参数和最近项目索引会保存到本地 `.local/state.sqlite3`，后端重启后仍可查看；该状态库不保存小说正文或 API Key。章节草稿/定稿为兼容旧 GUI，会在输出目录根部 `chapter_X.txt` 与旧函数使用的 `chapters/chapter_X.txt` 之间同步正文。

新前端读类接口允许后端不可用时降级到 mock 数据，但页面会标记为“离线预览”。离线预览或断线状态下，保存、导入、清理向量库、创建生成任务、WebDAV 操作等写类入口应禁用或提前提示不可写。

## 新前端真实可用开发顺序

当前新前端已经能真实读写配置、核心项目文件、章节文件、知识导入文件、角色库文件和 WebDAV 配置；设定/目录、单章草稿/定稿已接入真实生成链路，生成任务历史和最近项目索引已落到本地 SQLite 状态库；章节列表可显示从目录或章节数推导出的计划章节，并可从前端创建缺失的 `chapter_X.txt`；知识导入已接入当前 Embedding 配置并写入 Chroma 向量库；项目页已支持新建、打开和切换本地项目。继续开发时不要优先扩大视觉重构范围，应继续补齐审校、批量、项目配置隔离等真实创作能力。
...
5. **项目管理能力**
   ...
   - 当前状态：已完成最小闭环；当前项目仍由根目录 `config.json.other_params.filepath` 决定，项目页可创建指定输出目录、打开已有目录和切换最近项目；最近项目索引保存到 `.local/state.sqlite3`。后续可补每项目独立模型配置、项目重命名/归档和远程同步。
   - 后端重点：当前提供项目元数据、最近项目列表、创建项目、切换当前项目接口，同时兼容已有输出目录格式。
...
- 项目页能显示当前项目摘要。
- 项目页可以新建临时项目输出目录，例如 `/tmp/ai-novel-acceptance-project`，成功后当前输出路径切换到该目录。
- 项目页可以打开已有输出目录 `/tmp/ai-novel-acceptance`，成功后最近项目列表显示该目录且当前输出路径切回该目录。
- 工作台左侧能看到核心项目文件和章节。
