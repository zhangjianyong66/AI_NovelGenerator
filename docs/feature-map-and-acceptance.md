...
当前新前端已经能真实读写配置、核心项目文件、已存在章节、知识导入文件、角色库文件和 WebDAV 配置；设定/目录、单章草稿/定稿已接入真实生成链路，生成任务历史已落到本地 SQLite 状态库。继续开发时不要优先扩大视觉重构范围，应继续补齐知识检索、项目管理和章节生命周期能力。
...
6. **章节生命周期补齐**
   ...
   - 后端重点：根据目录创建章节记录和空章节文件，维护章节状态：未开始、草稿、已定稿、需审校。
   - 前端重点：章节编辑页支持创建章节、查看章节状态、关联 `outline_X.txt`。
...
- 缺失章节保存失败时，应提示需要先准备对应的 `chapter_X.txt`。
