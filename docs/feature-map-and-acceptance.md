...
| 章节定稿 | 润色并更新章节、摘要、角色状态和向量库 | 完整支持 | 真实支持，需要已有章节正文和有效 LLM 配置 | 同上，`finalization` 以根目录 `chapter_X.txt` 为输入，同步到旧 `chapters/` 后基于前章结尾、当前/下一章目录润色正文并写回 | 编辑章节后创建定稿任务，检查章节正文、`global_summary.txt` 和 `character_state.txt` |
| 批量草稿 | 按章节范围生成缺失草稿 | 通过逐章操作支持 | 真实支持，需要有效 LLM 配置和章节目录 | 同上，`batchDraft` 逐章复用 `draft`；遇到已有 `chapter_X.txt` 会跳过不覆盖，单章失败后继续后续章节 | 填写范围后创建批量草稿，检查缺失章节生成且已有章节未被覆盖 |
| 批量定稿 | 按章节范围定稿已有章节 | 通过逐章操作支持 | 真实支持，需要范围内章节文件和有效 LLM 配置 | 同上，`batchFinalization` 执行已有章节批量定稿，兼容旧 `batch`；逐章复用 `finalization`，单章失败后继续后续章节 | 准备多个 `chapter_X.txt` 后创建批量定稿任务，检查成功章节正文和状态文件更新 |
| 批量审校 | 按章节范围检查逻辑冲突 | 通过逐章操作支持 | 真实支持，需要范围内章节文件和有效审校 LLM 配置 | 同上，`batchConsistency` 逐章复用 `consistency`；审校结果写入任务日志且不修改正文文件 | 准备多个 `chapter_X.txt` 后创建批量审校任务，查看任务日志中的逐章审校结果 |
| 一致性审校 | 检查章节逻辑冲突 | 完整支持 | 真实支持，需要已有章节正文和有效审校 LLM 配置 | 同上，`consistency` 同步调用旧审校函数，结果写入任务日志且不修改正文文件 | 创建审校任务，查看任务日志中的审校结果 |
...
重要边界：新前端生成任务当前“小说设定”“章节目录”“章节草稿”“章节定稿”“批量草稿”“批量定稿”“批量审校”和“一致性审校”已接入同步真实执行器；定稿会基于前章结尾、当前/下一章目录信息润色章节正文并写回 `chapter_X.txt`，再更新摘要、角色状态和向量库；批量草稿会生成范围内缺失 `chapter_X.txt`，遇到已有章节会跳过不覆盖；批量定稿和批量审校只处理范围内已有 `chapter_X.txt`。审校会读取小说设定、角色状态、全局摘要、剧情要点和当前章节正文，结果写入任务日志，不自动修改章节或项目文件。生成任务历史、日志、错误、创建请求参数、最近项目索引和最近项目参数快照会保存到本地 `.local/state.sqlite3`，后端重启后仍可查看；该状态库不保存小说正文或 API Key。章节草稿/定稿为兼容旧 GUI，会在输出目录根部 `chapter_X.txt` 与旧函数使用的 `chapters/chapter_X.txt` 之间同步正文。

新前端读类接口允许后端不可用时降级到 mock 数据，但页面会标记为“离线预览”。离线预览或断线状态下，保存、导入、清理向量库、创建生成任务、WebDAV 操作等写类入口应禁用或提前提示不可写。

## 新前端真实可用开发顺序

当前新前端已经能真实读写配置、核心项目文件、章节文件、知识导入文件、角色库文件和 WebDAV 配置；设定/目录、单章草稿/定稿、批量草稿/定稿/审校和一致性审校已接入真实生成链路，生成任务历史、最近项目索引和项目参数快照已落到本地 SQLite 状态库；章节列表可显示从目录或章节数推导出的计划章节，并可从前端创建缺失的 `chapter_X.txt`；知识导入已接入当前 Embedding 配置并写入 Chroma 向量库；项目页已支持新建、打开和切换本地项目，且第一版已隔离项目参数，模型配置和密钥仍全局共享。继续开发时不要优先扩大视觉重构范围，应继续补齐后台任务、重试恢复、每项目模型覆盖等真实创作能力。

### 后续任务路线

当前下一阶段应优先从“同步可用”推进到“长任务稳定可用”，不要优先扩大纯视觉 UI 重构范围。

1. **生成任务后台执行**
   - 目标：`POST /api/generation-jobs` 只负责创建任务并快速返回，实际 LLM 调用由后台 worker 执行。
   - 后端重点：保留现有 `.local/state.sqlite3` 任务记录，新增后台执行状态流转；同步执行器可先作为 worker 内部调用，不急于重写生成核心。
   - 前端重点：生成页通过轮询或后续事件机制刷新任务状态、进度、日志和错误；创建任务后不阻塞页面。
   - 验收：创建设定、目录、草稿、定稿、批量草稿、批量定稿、批量审校或审校任务后，接口快速返回 `queued/running`，前端稍后能看到 `done/failed` 和完整日志。

2. **失败任务重试与批量失败重跑**
   - 目标：用户可以对 `failed` 任务发起重试；批量任务优先支持只重跑失败章节。
   - 后端重点：利用 `generation_jobs.request_json` 保存的原始请求参数，新增重试接口或重试动作；批量重试要保留原章节范围、目标字数、最低字数和自动扩写设置。
   - 前端重点：任务详情提供明确的“重试”入口，并在批量任务日志中区分成功章节、失败章节和本次重跑章节。
   - 验收：人为制造某章失败后，修复条件并重试，失败章节可重新执行且新日志可追溯。

3. **任务中断恢复边界**
   - 目标：后端进程重启后，不让历史 `running` 任务假装仍在执行。
   - 后端重点：启动时扫描状态库，把上次进程遗留的 `running` 任务标记为“中断/失败”并写入中文日志；已完成和已失败任务保持可读。
   - 前端重点：任务列表和详情能展示“中断”原因，引导用户重试。
   - 验收：任务运行中终止后端，再重启后该任务不再卡在 `running`，详情日志说明后端重启导致执行中断。

4. **每项目模型配置覆盖**
   - 目标：在当前项目参数隔离基础上，允许项目选择自己的阶段模型或 Embedding 配置覆盖，但 API Key 和全局配置仍要谨慎处理。
   - 后端重点：先设计项目级覆盖字段和旧 GUI 兼容策略；根目录 `config.json` 仍是旧 GUI 当前活动配置，不能让密钥位置变得不可预测。
   - 前端重点：设置页明确区分“全局模型配置”和“当前项目覆盖”，避免用户误以为密钥被复制到项目目录。
   - 验收：项目 A 和项目 B 可选择不同阶段模型；切换项目后生成任务使用对应项目覆盖；未配置覆盖时回退全局配置。

5. **章节生命周期状态补强**
   - 目标：在不迁移正文到数据库的前提下，补充“草稿、已定稿、需审校、审校完成”等可恢复状态。
   - 后端重点：优先记录轻量状态元数据，不改变 `chapter_X.txt` 仍是正文权威来源的边界。
   - 前端重点：章节页和生成页能按状态提示下一步操作，例如“已生成草稿，可定稿”“定稿后建议审校”。
   - 验收：章节状态可跨后端重启保留，并且不会影响旧 GUI 继续读写章节文本。
...
2. **章节草稿与定稿闭环**
   ...
   - 当前状态：单章草稿、单章定稿、批量草稿、批量定稿、批量审校和一致性审校已接入同步执行器；`plot_arcs.txt` 自动更新仍待后续 milestone。
   - 后端重点：`draft` 生成 `chapter_X.txt`，`finalization` 润色并写回 `chapter_X.txt`、更新 `global_summary.txt`、`character_state.txt` 并沿用旧逻辑尝试更新 `vectorstore/`；`consistency` 调用旧审校函数并把结果写入任务日志；同步根部 `chapter_X.txt` 与旧 `chapters/chapter_X.txt`。
   - 前端重点：明确目标章节、文件变更结果、失败原因和重试入口。
   - 验收：从前端生成第 1 章草稿后出现非空 `chapter_1.txt`；定稿后章节正文被润色写回，摘要或角色状态发生合理更新。
...
5. **项目管理能力**
   ...
   - 当前状态：已完成最小闭环；当前项目仍由根目录 `config.json.other_params.filepath` 决定，项目页可创建指定输出目录、打开已有目录和切换最近项目；最近项目索引和项目参数快照保存到 `.local/state.sqlite3`，切换项目会保存当前项目参数并恢复目标项目参数。打开尚无参数快照的已有目录时，只切换输出路径并清空项目参数，避免继承上一个项目的题材和章节设置。模型配置、密钥、代理和 WebDAV 仍全局共享。后续可补每项目独立模型覆盖、项目重命名/归档和远程同步。
   - 后端重点：当前提供项目元数据、最近项目列表、创建项目、切换当前项目接口，同时兼容已有输出目录格式，并保持根目录 `config.json` 作为旧 GUI 的当前活动配置。
...
- 批量章节参数可填写起始章节 `1`、结束章节 `1`、目标字数 `1200`、最低字数 `800`，分别创建批量草稿、批量定稿或批量审校任务。
- 查看任务列表和详情日志。

验收标准：

- 无 API Key 时，设定、目录、草稿、定稿和审校任务状态应为 `failed`，错误说明缺少 API Key。
- 批量草稿会生成缺失章节并跳过已有章节；批量定稿和批量审校会处理已有章节；有效配置下最终为 `done`，部分章节失败时最终为 `failed` 并汇总成功/失败章节。
- 详情页不应再把所有任务都描述为“不会调用 LLM 或写入小说文件”；该边界只适用于未接入阶段。
- 设定、目录、草稿、定稿和审校任务日志应包含真实执行开始、完成或失败原因；审校成功日志应包含审校结果。
- 批量任务日志应包含章节范围、目标字数、最低字数、自动扩写设置和逐章草稿/定稿/审校结果。
...
- 如果批量定稿或批量审校范围包含不存在的章节，例如只存在 `chapter_1.txt` 时填写 `1-2`，页面或后端应提示缺少 `chapter_2.txt`；同样范围创建批量草稿时应允许生成缺失章节。
...
这套验收用于旧 GUI，或新前端已接入真实执行器的阶段。当前新前端可验收“生成小说设定”“生成章节目录”“生成章节草稿”“章节定稿”“批量草稿”“批量定稿”“批量审校”“一致性审校”和“知识库向量化导入”。执行前需要有效 LLM 配置；涉及知识库导入和检索时还需要可用 Embedding。
...
7. 创建“批量草稿”任务，确认缺失章节会生成，已有 `chapter_X.txt` 不会被覆盖。
8. 准备多个已有章节后创建“批量定稿”任务，确认批量定稿逐章处理已有 `chapter_X.txt`，日志汇总成功/失败章节。
9. 创建“批量审校”或单章“审校”任务，确认任务日志输出审校结果；审校不会自动修改章节正文、摘要、角色状态或剧情要点文件。
...
| 创建草稿/定稿/审校任务失败 | 当前章节号无效、章节文件不存在或阶段模型配置缺失 | 设置当前章节为 `1`，准备 `chapter_1.txt`，并检查对应 LLM 配置 |
| 批量任务失败 | 批量定稿/审校范围内章节缺失或为空、阶段 LLM 配置缺失或某章生成/定稿/审校调用失败 | 检查批量任务日志中的逐章失败原因；成功章节会继续处理，失败章节需修复后重跑 |
