...
当前新前端已经能真实读写配置、核心项目文件、已存在章节、知识导入文件、角色库文件和 WebDAV 配置；设定和目录阶段已开始接入真实生成链路。继续开发时不要优先扩大视觉重构范围，应继续补齐章节生成、任务持久化和知识检索能力。
...
3. **任务持久化与可恢复工作流**
   ...
   - 后端重点：用 SQLite 保存任务、阶段、输入参数、进度、日志、错误和输出文件路径；长流程失败后允许从失败步骤重试。
   - 前端重点：任务列表显示历史任务，详情页展示可恢复状态和重试操作。
   - 验收：执行中断或后端重启后，前端仍能看到任务记录，并能继续或重试失败步骤。
