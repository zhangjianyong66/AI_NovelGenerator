...
重要边界：新前端生成任务当前“小说设定”“章节目录”“章节草稿”“章节定稿”和“一致性审校”已接入同步真实执行器；批量生成仍只创建任务记录，不会调用 LLM 或修改小说文件。审校会读取小说设定、角色状态、全局摘要、剧情要点和当前章节正文，结果写入任务日志，不自动修改章节或项目文件。生成任务历史、日志、错误、创建请求参数和最近项目索引会保存到本地 `.local/state.sqlite3`，后端重启后仍可查看；该状态库不保存小说正文或 API Key。章节草稿/定稿为兼容旧 GUI，会在输出目录根部 `chapter_X.txt` 与旧函数使用的 `chapters/chapter_X.txt` 之间同步正文。

新前端读类接口允许后端不可用时降级到 mock 数据，但页面会标记为“离线预览”。离线预览或断线状态下，保存、导入、清理向量库、创建生成任务、WebDAV 操作等写类入口应禁用或提前提示不可写。

## 新前端真实可用开发顺序

当前新前端已经能真实读写配置、核心项目文件、章节文件、知识导入文件、角色库文件和 WebDAV 配置；设定/目录、单章草稿/定稿和一致性审校已接入真实生成链路，生成任务历史和最近项目索引已落到本地 SQLite 状态库；章节列表可显示从目录或章节数推导出的计划章节，并可从前端创建缺失的 `chapter_X.txt`；知识导入已接入当前 Embedding 配置并写入 Chroma 向量库；项目页已支持新建、打开和切换本地项目。继续开发时不要优先扩大视觉重构范围，应继续补齐批量、项目配置隔离等真实创作能力。
...
2. **章节草稿与定稿闭环**
   ...
   - 当前状态：单章草稿、单章定稿和一致性审校已接入同步执行器；批量生成和 `plot_arcs.txt` 自动更新仍待后续 milestone。
...
这套验收用于旧 GUI，或新前端已接入真实执行器的阶段。当前新前端可验收“生成小说设定”“生成章节目录”“生成章节草稿”“章节定稿”“一致性审校”和“知识库向量化导入”；批量生成仍按旧 GUI 或后续 milestone 验收。执行前需要有效 LLM 配置；涉及知识库导入和检索时还需要可用 Embedding。
...
7. 创建“审校”任务，确认任务日志输出审校结果；审校不会自动修改章节正文、摘要、角色状态或剧情要点文件。
...
| 批量任务一直是 `queued` | 批量阶段尚未接入真实执行器 | 这是当前预期行为，不代表单章生成或审校能力失败 |
