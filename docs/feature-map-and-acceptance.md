...
| 批量定稿 | 按章节范围定稿已有章节 | 通过逐章操作支持 | 真实支持，需要范围内章节文件和有效 LLM 配置 | 同上，`batch` 第一版执行已有章节批量定稿，逐章复用 `finalization`；单章失败后继续后续章节，最终汇总成功/失败 | 准备多个 `chapter_X.txt` 后创建批量任务，检查成功章节正文和状态文件更新 |
...
重要边界：新前端生成任务当前“小说设定”“章节目录”“章节草稿”“章节定稿”“批量定稿”和“一致性审校”已接入同步真实执行器；批量第一版只定稿范围内已有 `chapter_X.txt`，不会自动生成缺失草稿。审校会读取小说设定、角色状态、全局摘要、剧情要点和当前章节正文，结果写入任务日志，不自动修改章节或项目文件。生成任务历史、日志、错误、创建请求参数、最近项目索引和最近项目参数快照会保存到本地 `.local/state.sqlite3`，后端重启后仍可查看；该状态库不保存小说正文或 API Key。章节草稿/定稿为兼容旧 GUI，会在输出目录根部 `chapter_X.txt` 与旧函数使用的 `chapters/chapter_X.txt` 之间同步正文。

新前端读类接口允许后端不可用时降级到 mock 数据，但页面会标记为“离线预览”。离线预览或断线状态下，保存、导入、清理向量库、创建生成任务、WebDAV 操作等写类入口应禁用或提前提示不可写。

## 新前端真实可用开发顺序

当前新前端已经能真实读写配置、核心项目文件、章节文件、知识导入文件、角色库文件和 WebDAV 配置；设定/目录、单章草稿/定稿、批量定稿和一致性审校已接入真实生成链路，生成任务历史、最近项目索引和项目参数快照已落到本地 SQLite 状态库；章节列表可显示从目录或章节数推导出的计划章节，并可从前端创建缺失的 `chapter_X.txt`；知识导入已接入当前 Embedding 配置并写入 Chroma 向量库；项目页已支持新建、打开和切换本地项目，且第一版已隔离项目参数，模型配置和密钥仍全局共享。继续开发时不要优先扩大视觉重构范围，应继续补齐后台任务、重试恢复、每项目模型覆盖等真实创作能力。
...
1. **生成任务后台执行**
   ...
   - 验收：创建设定、目录、草稿、定稿、批量定稿或审校任务后，接口快速返回 `queued/running`，前端稍后能看到 `done/failed` 和完整日志。

2. **失败任务重试与批量失败重跑**
   - 目标：用户可以对 `failed` 任务发起重试；批量定稿优先支持只重跑失败章节。
...
2. **章节草稿与定稿闭环**
   ...
   - 当前状态：单章草稿、单章定稿、批量定稿和一致性审校已接入同步执行器；`plot_arcs.txt` 自动更新仍待后续 milestone。
   - 后端重点：`draft` 生成 `chapter_X.txt`，`finalization` 更新 `global_summary.txt`、`character_state.txt` 并沿用旧逻辑尝试更新 `vectorstore/`；`consistency` 调用旧审校函数并把结果写入任务日志；同步根部 `chapter_X.txt` 与旧 `chapters/chapter_X.txt`。
...
这套验收用于旧 GUI，或新前端已接入真实执行器的阶段。当前新前端可验收“生成小说设定”“生成章节目录”“生成章节草稿”“章节定稿”“批量定稿”“一致性审校”和“知识库向量化导入”。执行前需要有效 LLM 配置；涉及知识库导入和检索时还需要可用 Embedding。
...
7. 准备多个已有章节后创建“批量”任务，确认批量定稿逐章处理已有 `chapter_X.txt`，日志汇总成功/失败章节。
8. 创建“审校”任务，确认任务日志输出审校结果；审校不会自动修改章节正文、摘要、角色状态或剧情要点文件。
...
| 批量任务失败 | 范围内章节缺失/为空、定稿 LLM 配置缺失或某章定稿调用失败 | 检查批量任务日志中的逐章失败原因；成功章节会继续处理，失败章节需修复后重跑 |
